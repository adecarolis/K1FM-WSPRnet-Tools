...
def decimal_to_aprs(deg, latlng):
    ...
    if latlng == 'lat' and d >= 0:
        suffix = 'N'
    elif latlng == 'lat' and d < 0:
        suffix = 'S'

    if latlng == 'lng' and d >= 0:
        suffix = 'E'
    elif latlng == 'lng' and d < 0:
        suffix = 'W'
...
def generate_aprs_string(wspr_data):
    ...
    return "{}-11>{}:/{}z{}/{:0>9s}O{} Solar:{}V Temperature:{}" \
           "C Satellites:{} /A={:0>6d}".format(
               wspr_data['callsign'],
               path,
               time,
               lat,
